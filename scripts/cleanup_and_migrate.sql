-- Supabase 정리 및 마이그레이션 스크립트
-- 기존 불필요한 테이블 제거 후 새로운 통합 스키마 생성
...
-- 2. 기존 테이블들 제거 (CASCADE로 의존성까지 제거)
...
DO $$
BEGIN
    RAISE NOTICE '🎉 Supabase 정리 및 마이그레이션 완료!';
    RAISE NOTICE '✅ 기존 테이블 제거: rules, recommendations, med_alias_map 등';
    RAISE NOTICE '✅ 새로운 테이블 생성: products, ingredients, users, user_profiles 등';
    RAISE NOTICE '✅ JSONB 최적화: tags, purposes, skin_concerns, intent_tags';
    RAISE NOTICE '✅ GIN 인덱스: JSONB 필드 검색 최적화';
    RAISE NOTICE '✅ UUID 기반 사용자 시스템 준비 완료';
    RAISE NOTICE '🚀 이제 SQLite 데이터를 마이그레이션할 준비가 되었습니다!';
END $$;